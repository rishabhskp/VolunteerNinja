...
function setValTo(getDat){
    $('#to').val(getDat);
    $('#calendar1').hide();
}
            
function dispCalTo(mon,yea){
	var ar = new Array('Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec');
    var chkEmpty = isEmpty(mon);
    var n,days,calendar,startDate,newYea,setvale,i;
    if(chkEmpty != true){
	...
	while (i <= days){
		...
		setvale = i+","+n;
		if(i == day && newYea==yeare && mon==monthe){
			calendar +="<td class='thisday' onclick='setValTo(\""+i+"-"+mon+"-"+newYea+"\")'>"+i+"</td>";
		}
		else{  
    		calendar +="<td class='thismon' onclick='setValTo(\""+i+"-"+mon+"-"+newYea+"\")'>"+i+"</td>";   
    		}
		startDay1++;  
    	i++;  
    }
